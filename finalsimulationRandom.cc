...
ReceivePacket (Ptr<Socket> socket)
{
   ...
   while(packet = socket->Recv())
    {
        rcv = Simulator::Now().GetNanoSeconds(); // cas pri prijati paketu
        SeqTsHeader seqTsx;
        ...
        packetsReceived += 1;
        delay = delay +  (rcv - sqhd); 
    }
    xx = rcv - senttime;    
    throughput = (bytesTotal * 8 * 1000000000) / (1024 * xx);

    // funkcia bola doplnena o zapis priepustnosti (throughput) v case do csv suboru
    std::fstream fout;
...
main (int argc, char *argv[])
{
  ...
  uint32_t packetSize = 64*2; // bytes
  //uint32_t numPackets = 1000; now defined as global variable for throughput calculations
  uint32_t numNodes = 25; // by default, 5x5
  //pre dynamicky model boli pouzite aj nasledovne dva parametre, rychlost a pozastavenie uzla - nepouzivame
  ...
  //pre parsovanie parametrov priamo cez CLI
  CommandLine cmd;
  ...
  */
  ...
  //pridane velkosti paketu a data rate pre co najpresnejsie porovnanie
  ...
  wifiPhy.Set ("RxGain", DoubleValue (-1.5));
  wifiPhy.Set ("TxGain", DoubleValue (1.5));
  ...
  //wifiPhy.Set ("TxPowerStart",DoubleValue (1.5));
  //wifiPhy.Set ("TxPowerEnd", DoubleValue (1.5));
  // Set it to adhoc mode
  wifiMac.SetType ("ns3::AdhocWifiMac");
  NetDeviceContainer devices = wifi.Install (wifiPhy, wifiMac, c);

  MobilityHelper mobility;
  //potrebne pre pohyb uzlov
  ...
  //povodny kod pre staticky model - odkomentovat pre pouzitie statickeho modelu
  mobility.SetPositionAllocator ("ns3::RandomRectanglePositionAllocator",
                                  "X", StringValue ("ns3::UniformRandomVariable[Min=0|Max=100]"),
                                  "Y", StringValue ("ns3::UniformRandomVariable[Min=0|Max=100]"));
  
  //pre hybajuce sa uzly - doplneny dynamicky model - odkomentovat pre pouzitie
  ...
  //pre staticku polohu uzlov - pre pouzitie odkomentovat
  //mobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
  mobility.Install (c);
  //potrebne pre pohyb ulov
  ...
  for(NetDeviceContainer :: Iterator it = devices.Begin(); it!=devices.End(); it++){
        ...
        //radioEnergyHelper.Set ("TxCurrentA", DoubleValue (7.5));
 ...
 // double stopTime = 120.0;
  ...
  AnimationInterface anim ("animation_AOMDV.xml");
  ...
  NS_LOG_UNCOND("Total sent packets = "<<numPackets);
  NS_LOG_UNCOND("Total received packets = "<<packetsReceived);
  NS_LOG_UNCOND("Total lost packets = "<<numPackets-packetsReceived);
  NS_LOG_UNCOND("Packet delivery ratio= "<< ((packetsReceived*100)/numPackets) <<"%");
  ...
  //tieto hodnoty su irelevantne vzhladom na to, ze praca je zamerana na odolnost prenosu, nie na spotrebu energie, preto boli zakomentovane
